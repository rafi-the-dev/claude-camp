...
const { getTarget, CAMP_COMMANDS } = require('./targets');
...
async function uninstall(opts = {}) {
  ...
  if (opts.yes) {
    ...
    console.log(`Removing (agent: ${target.label}):`);
  ...
  try {
    const instructionsLabel = config.claudeMode === 'separate' ? target.splitLabel : target.mergeLabel;
    console.log(`Camp installed the following (agent: ${target.label}):`);
    console.log(`  1. Wiki commands     (${CAMP_COMMANDS.length} files in ${target.commandsDir})`);
    console.log(`  2. ${instructionsLabel} (wiki section)`);
    ...
    const removeClaude = await question(rl, `  Remove wiki section from ${instructionsLabel}? [y/N] `);
    ...
    if (removeCmds) removeCommands(target);
    else console.log('  Skipped wiki commands');

    if (removeClaude) removeClaudeMdSection(config, target);
