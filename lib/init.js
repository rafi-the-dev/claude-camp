...
async function init(opts = {}) {
  ...
  if (opts.yes) {
    ...
    try {
      ...
      const claudeMode = await question(rl, `Merge wiki section into ${claudeMdPath}, or create ${campMdPath}? (merge/separate)`, 'merge');
      const userName = await question(rl, 'Your name (for about-me.md):', '');

      config = {
        vaultPath: path.resolve(vaultPath),
        templatePath: path.resolve(templatePath),
        claudeMode: claudeMode === 'separate' ? 'separate' : 'merge',
        userName,
        platform: process.platform,
      };
    } finally {
  ...
  const modePath = config.claudeMode === 'separate' ? campMdPath : claudeMdPath;
  console.log(`  CLAUDE.md configured (${config.claudeMode} mode) → ${modePath}`);
  ...
  console.log(`  CLAUDE.md:   ${modePath} (${config.claudeMode})`);
  console.log(`  Commands:    9 wiki commands in ${commandsDir}\n`);
...
function configureClaudeMd(config) {
  ...
  const targetPath = config.claudeMode === 'separate'
    ? path.join(os.homedir(), 'CAMP.md')
    : path.join(os.homedir(), 'CLAUDE.md');

  if (config.claudeMode === 'merge' && fs.existsSync(targetPath)) {
    ...
    fs.writeFileSync(targetPath, wikiSection);
  }
}
