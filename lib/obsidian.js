...
function doctor() {
  ...
  if (config && config.claudeMode === 'separate') {
    ...
    // Check that CLAUDE.md references CAMP.md
    if (fs.existsSync(claudePath)) {
      const claudeContent = fs.readFileSync(claudePath, 'utf8');
      if (claudeContent.includes('CAMP.md')) {
        console.log(`  [OK]   ${claudePath} references CAMP.md`);
      } else {
        console.log(`  [FAIL] ${claudePath} does not reference CAMP.md`);
